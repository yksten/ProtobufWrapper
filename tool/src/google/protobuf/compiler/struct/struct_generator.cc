...
bool StructGenerator::Generate(const FileDescriptor* file, const string& parameter, GeneratorContext* generator_context, string* error) const {
    ...
    google::protobuf::scoped_ptr<io::ZeroCopyOutputStream> outputSrc(generator_context->Open(basename + ".pb.cc"));
    io::Printer printerSrc(outputSrc.get(), '$', NULL);
    obj.printSource(printerSrc, basename.c_str());
