...
namespace proto {
    ...
    public:
        ...
        bool bind(void(*f)(T&, const P&, const uint32_t, bool*), serialization::serializeItem<T>& value) {
        ...
        template<typename P, typename T>
        bool bind(void(*f)(std::vector<T>&, const P&, const uint32_t, bool*), serialization::serializeItem<std::vector<T> >& value) {
            return _functionSet.insert(std::pair<uint32_t, converter>(value.num, converter(convert_t(f), &value.value, value.type, value.bHas))).second;
        }

        template<typename P, typename K, typename V>
        bool bind(void(*f)(std::map<K, V>&, const P&, const uint32_t, bool*), serialization::serializeItem<std::map<K, V> >& value) {
            return _functionSet.insert(std::pair<uint32_t, converter>(value.num, converter(convert_t(f), &value.value, value.type, value.bHas))).second;
        }
