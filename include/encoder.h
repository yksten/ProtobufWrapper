...
namespace serialize {
    ...
    static inline uint64_t VarintByteSize(uint64_t value) {
        uint64_t i = 0;
        while (value >= 0x80) {
            ++i; value >>= 7;
        }
    ...
    static inline uint64_t SvarintByteSize(uint32_t value) {
        return VarintByteSize(static_cast<uint64_t>(((value << 1) ^ static_cast<uint32_t>(value >> 31))));
    }
    static inline uint64_t SvarintByteSize(uint64_t value) {
        return VarintByteSize((value << 1) ^ static_cast<uint64_t>(value >> 63));
    }

    static inline uint64_t ByteSize(const std::string& value, uint32_t type = serialize::TYPE_VARINT) {
        return VarintByteSize(static_cast<uint64_t>(value.length())) + value.length();
    }
    
    static inline uint64_t ByteSize(uint64_t value, uint32_t type = serialize::TYPE_VARINT) {
        if (type == TYPE_VARINT) {
            return VarintByteSize(static_cast<uint64_t>(value));
        } else if (type == TYPE_SVARINT) {
            SvarintByteSize(value);
    ...
    template <typename T>
    static inline uint64_t ByteSize(const T& value, uint32_t type = serialize::TYPE_VARINT) {
        uint64_t customLength = value.getByteSize();
        return VarintByteSize(customLength) + customLength;
    }
    ...
    private:
        ...
        static void encodeValue(const std::map<K, V>& value, const bool* pHas, const enclosure_t& info, BufferWrapper& buf) {
            for (typename std::map<K, V>::const_iterator it = value.begin(); it != value.end(); ++it) {
                ...
                nByteSize += infok.size + serialize::ByteSize(typename internal::ByteSizeTypeTraits<K>::Type(it->first), keyType);
                uint32_t valueType = info.type & 0xFFFF;
                nByteSize += infov.size + serialize::ByteSize(typename internal::ByteSizeTypeTraits<V>::Type(it->second), valueType);
                varInt(nByteSize, buf);
