...
/*=====================================message====================================*/
    ...
    bool Message::ParseFromBytes(uint8_t* szBin, size_t nBinSize) {
        uint8_t* current = szBin;
    ...
    uint64_t Message::GetSInt(uint32_t number) {
        ...
        if (Field* field = GetField(number)) {
            ...
            nRet = static_cast<uint64_t>((field->_uint64s.front() >> 1) ^ (-(field->_uint64s.front() & 1)));
        }
        return nRet;
    }
    ...
    uint64_t Message::GetSFixedInt64(uint32_t number) {
        ...
        if (Field* field = GetField(number)) {
            assert(field->type == Field::FIELD_UINT64);
            nRet = static_cast<uint64_t>((field->_uint32s.front() >> 1) ^ (-(field->_uint32s.front() & 1)));
        }
        return nRet;
    }
    ...
    bool Message::GetByteArray(uint32_t number, std::vector<std::pair<uint8_t*, size_t> >& value) {
        bool result = false;
        if (Field* field = GetField(number)) {
            assert(field->type == Field::FIELD_UINT64);
            for (uint32_t idx = 0; idx < field->_bins.size(); ++idx) {
