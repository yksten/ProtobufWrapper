...
namespace picoproto {
        ...
        case FIELD_BYTES: {
            if (owns_data) {
                ...
                for (std::pair<uint8_t*, size_t> data_info : *other.value.v_bytes) {
                    ...
                    value.v_bytes->push_back(std::pair<uint8_t*, size_t>(new_data, data_info.second
                        ));
                }
            } else {
                value.v_bytes =
    ...
    bool Message::ParseFromBytes(uint8_t* bytes, size_t bytes_size) {
        ...
        while (remaining > 0) {
            ...
            case WIRETYPE_LENGTH_DELIMITED: {
                ...
                field->value.v_bytes->push_back(std::pair<uint8_t*, size_t>(data, size));
    ...
    Field* Message::AddField(int32_t number, enum FieldType type) {
        ...
        if (!field) {
            ...
            field = &fields.back();
            field_map.insert(std::map<int32_t, size_t>::value_type(number, fields.size() - 1
                ));
        }
        return field;
    }
